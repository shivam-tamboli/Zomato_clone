...
export default class RateOrder extends Component {
    ...
    constructor(props){
        ...
        this.obj.orderFoodItems.forEach((value, index)=>{
    ...
    submitReview = (e) =>{
        ...
        this.rateObj.fooditemid.forEach((val,i)=>{
            this.rateObj.fooditemid[i]=String(val)
        })
        axios.post("http://localhost:8080/zomato/user/rate-order", this.rateObj)
        .then((resp)=>{
            History.push({
                pathname:"Orders",
                state:{
                            ...
                            this.obj.orderFoodItems.map((value, index)=>{
                                return(
                                    <div key={index} className='ordfood'>
                                        ...
                                        <input type="text" onChange={this.reviewEnter} id={'food'+index} placeholder="Enter Food Review" className='reviewtxt' />
                                    </div>
                                );
                            })
                        }
